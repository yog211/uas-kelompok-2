  ...
  <div class="row page-titles mx-0">
    <div class="col-sm-6 p-md-0">
      <div class="welcome-text">
        <span class="ml-1">Uang Masuk</span>
      </div>
    </div>
  ...
  <div class="row">
    <div class="col-12">
      <div class="card">
        ...
        <div class="card-body">
          <a href="{{ route('uang_masuk.create')}}" class="btn btn-primary">Create</a>
          <div class="table-responsive">
            <table id="table-uang" class="display nowrap text-dark" style="width:100%">
              ...
              <tbody>
                @foreach ($uang_masuks as $uang)
                <tr>
                  <td>{{ $loop->iteration }}</td>
                  <td>{{ $uang->sumber_uang_masuk }}</td>
                  <td>{{ $uang->nominal }}</td>
                  <td>{{ date('d-m-Y', strtotime($uang->tgl_masuk))}}</td>
                  <td>
                    <form action="{{ route('uang_masuk.delete', $uang->id)}}" method="post">
                      @csrf
                      @method('delete')
                      <a href="{{ route('uang_masuk.edit', $uang->id)}}"
